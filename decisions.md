...
**Alternatives considered:**
- Do both: keyword rule *and* let it flow into the classifier as a backstop. Considered, but
...
but narrower than it first looks: the *routing* decision is phrasing-based, the *answer* is
...
**Handling requests which may ask for both safe and restricted data in one prompt:** A partial-answer version would need to detect *which part* is restricted and answer only the rest, which is a much harder guarantee to make safely so it just flags entire request right now as restricted.